...
describe('isValidPin', () => {
  ...
  it('rejects non-string values', () => {
    expect(isValidPin(1234 as unknown as string)).toBe(false);
    expect(isValidPin(null as unknown as string)).toBe(false);
    expect(isValidPin(undefined as unknown as string)).toBe(false);
  });
});
