...
export default function RegisterScreen() {
  ...
  const { register, loginWithGoogleProvider, isGoogleAvailable, isLoading, error, clearError } = useAuth();
  ...
  const t = {
    ...
    passwordMismatch: language === 'tr' 
      ? 'Şifreler eşleşmiyor.' 
      : 'Passwords do not match.',
    fillAll: language === 'tr' 
      ? 'Lütfen tüm alanları doldurun.' 
      : 'Please fill all fields.',
    passwordMin: language === 'tr' 
  ...
  const handleRegister = async () => {
    ...
    if (!name.trim() || !email.trim() || !password.trim() || !confirmPassword.trim()) {
      Alert.alert(language === 'tr' ? 'Hata' : 'Error', t.fillAll);
      return;
    }

    if (password.length < 6) {
      Alert.alert(language === 'tr' ? 'Hata' : 'Error', t.passwordMin);
      return;
    }

    if (password !== confirmPassword) {
      Alert.alert(language === 'tr' ? 'Hata' : 'Error', t.passwordMismatch);
      return;
    }
