...
- **📅 K1'in SON adımı planlandı ama bilinçli olarak uygulanmadı:** `caregiverInvites` üzerindeki `allow get` hâlâ geniş ve brute-force enumeration ancak bu kural sahibiyle sınırlanınca **yapısal olarak** bitiyor (saldırı callable üzerinden değil doğrudan `getDoc` ile yürüyor, yani rate-limit onu durdurmuyor). Daraltma, kabul akışında `getDoc(invite)` yapan **sahadaki eski sürümleri anında kıracağı** için kademeli yayına bağlandı. Tam plan, eşikler, test değişiklikleri, doğrulama sırası ve geri alma adımları: **[`docs/DAVET_KODU_ALLOW_GET_DARALTMA_YAYILIM_PLANI.md`](./docs/DAVET_KODU_ALLOW_GET_DARALTMA_YAYILIM_PLANI.md)**.
  - Plan hazırlanırken doğrulanan iki kritik olgu: **(1)** mevcut istemcide `caregiverInvites`'a dokunan diğer tüm yollar `getDocs(query(...))` (`allow list`) veya `deleteDoc` (`allow delete`) — yani daraltma **güncel istemcide hiçbir şeyi kırmıyor**, yalnızca eski sürümleri. **(2)** ⚠️ **Sürüm yayılımı şu an ÖLÇÜLEMİYOR**: `APP_VERSION`/`useAppVersion` yalnızca Ayarlar → Hakkında gösterimi için kullanılıyor, sürümü Firestore'a/analitiğe yazan hiçbir yol yok ve Crashlytics üretimde aktif değil. Ölçüm ancak Play Console sürüm dağılımıyla mümkün; sıradaki sürümde `appVersion` telemetrisi eklenmesi öneriliyor.
  - Unutmama garantisi dokümanda değil **testte**: `inviteFlow.contract.test.ts`'in son assertion'ı mevcut (açık) durumu bilerek kilitliyor — biri erken daraltırsa da, zamanı gelip daraltırsa da test kırılır.
